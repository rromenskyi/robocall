...
func main() {
	...
	allowedIPs = parseAllowedIPs(config.Global.AllowedIP)
	...
	if config.AMI.Enabled == "1" {
		log.Println("AMI enabled")
		go initAMI(config.AMI)
	} else {
		log.Println("AMI disabled")
	}

	if config.OAMI.Enabled == "1" {
		log.Println("OAMI enabled")
...
func getSessionSecret() string {
	if secret := os.Getenv("ROBOCALL_SESSION_SECRET"); secret != "" {
		return secret
	}
...
func serveHTTP(config GlobalConfig, r *gin.Engine) error {
	...
	if httpPort != "" && httpsPort != "" {
		...
		return r.RunTLS(httpsPort, config.HttpsPubKey, config.HttpsPriKey)
	}
...
func setupRouter(config GlobalConfig) *gin.Engine {
	...
	r.GET("/user/:id", AuthRequired, func(c *gin.Context) {
		userId := c.Param("id")
		var user User
	...
	r.GET("/geo/:id", AuthRequired, func(c *gin.Context) {
		geoId := c.Param("id")
		var geo Geo
